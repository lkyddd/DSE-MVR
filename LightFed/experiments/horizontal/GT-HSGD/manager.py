...
class ClientManager(BaseServer):
    ...
    def fed_client_train_step(self, step):
        ...
        section_of_grad_y = formula(lambda y_t, u, u_old: y_t + u - u_old,
                                    self.trainer.grad_y,
                                    self.trainer.grad_u,
                                    self.trainer.grad_u_old)
    ...
    def fed_receive_neighbor_section_of_grad_y(self, nei_client_id, section_of_grad_y):
        ...
        if not self.unreceived_neighbor_num:
            ...
            self.trainer.grad_u_old = self.trainer.grad_u

            section_of_params = formula(lambda params_t, grad_y: params_t - self.gamma * grad_y,
