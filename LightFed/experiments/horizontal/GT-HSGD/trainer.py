...
class ClientTrainer:
    def __init__(self, args, client_id):
        ...
        self.grad_y = _zeros
        self.grad_u_old = _zeros

        batch_x, batch_y = self._new_random_batch()
        self.grad_u = self._get_grad_(self.model_params, batch_x, batch_y)
    ...
    def train_locally_step(self, step):
        ...
        self.grad_u = formula(lambda g_t_1, grad_u, g_t: g_t_1 + (1 - self.alpha) * (grad_u - g_t),
                              g_t_1, self.grad_u_old, g_t)
