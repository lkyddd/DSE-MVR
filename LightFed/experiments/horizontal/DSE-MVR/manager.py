...
class ClientManager(BaseServer):
    def __init__(self, ct, args):
        ...
        self.alpha_list = args.alpha_list.copy()
        self.alpha_list.append((math.inf, None))
        self.next_alpha_stage = -1
    ...
    def fed_client_train_step(self, step):
        ...
        self.trainer.model_params_mid = formula(lambda model_params, grad_v: model_params - self.gamma * grad_v,
                                                self.trainer.model_params, self.trainer.grad_v)
    ...
    def fed_receive_neighbor_section_of_params(self, nei_client_id, section_of_params):
        ...
        if not self.unreceived_neighbor_num:
            ...
            self.trainer.model_params_old = _agg_params
            self.trainer.model_params_tau_old = _agg_params
            self.trainer.reset_grad_v()
    ...
    def _set_alpha(self, step):
        if step >= self.next_alpha_stage:
            _, self.trainer.alpha = self.alpha_list.pop(0)
            self.next_alpha_stage = self.alpha_list[0][0]
            logging.info(f"update alpha to :{self.trainer.alpha}, next_alpha_stage:{self.next_alpha_stage}")
