        ...
        if ((step + 1) % self.args.tau) == 0:
            ...
            for nei_client_id in nei_client_id_list:
                if nei_client_id == self.client_id:
                    self.fed_receive_nei_section_of_model_params(self.client_id, self.trainer.model_params)
                else:
                    self._ct_.get_node("client", nei_client_id) \
                        .set(deepcopy=False) \
                        .fed_receive_nei_section_of_model_params(self.client_id, self.trainer.model_params)
                    self.communication_cost[self.client_id, nei_client_id] += model_size(self.trainer.model_params)

        else:
            self.trainer.calculate_grad_g()
            self.trainer.model_params = formula(lambda model_params, grad_g: model_params - self.gamma * grad_g,
                                                self.trainer.model_params,
                                                self.trainer.grad_g)
            self.trainer.grad_g = None

            self.finish_train_step()

    def fed_receive_nei_section_of_model_params(self, nei_client_id, nei_model_params):
        logging.debug(f"fed_receive_nei_section_of_model_params, nei_client_id:{nei_client_id}")

        weight = self.weight_matrix[nei_client_id, self.client_id]
        self.nei_params_aggregator.put(nei_model_params, weight)
        self.unreceived_neighbor_num -= 1

        if not self.unreceived_neighbor_num:

            logging.debug(f"all nei_model_params was received client:{self.client_id}, step:{self.step}")

            model_params_mid = self.nei_params_aggregator.get_and_clear()
            self.trainer.grad_m = formula(lambda grad_m, params_tau, params_mid: self.beta * grad_m + (params_tau - params_mid) / self.gamma,
                                          self.trainer.grad_m,
                                          self.trainer.model_params_tau,
                                          model_params_mid)
            self.trainer.model_params = formula(lambda params_tau, grad_m: params_tau - self.gamma * grad_m,
                                                self.trainer.model_params_tau,
                                                self.trainer.grad_m)
            self.trainer.model_params_tau = self.trainer.model_params
