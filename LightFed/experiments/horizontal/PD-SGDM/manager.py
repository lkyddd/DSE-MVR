...
class ClientManager(BaseServer):
    ...
    def fed_client_train_step(self, step):
        ...
        self.trainer.model_params_mid = formula(lambda p_t, g_u: p_t - self.gamma * g_u,
                                                self.trainer.model_params, self.trainer.grad_u)
        ...
        if ((step + 1) % self.args.tau) == 0:
            ...
            for nei_client_id in nei_client_id_list:
                if nei_client_id == self.client_id:
                    self.fed_receive_neighbor_section_of_params(self.client_id, self.trainer.model_params_mid)
                else:
                    self._ct_.get_node("client", nei_client_id) \
                        .set(deepcopy=False) \
                        .fed_receive_neighbor_section_of_params(self.client_id, self.trainer.model_params_mid)
    ...
    def fed_receive_neighbor_section_of_params(self, nei_client_id, section_of_params):
        logging.debug(f"fed_receive_neighbor_section_of_params_u, nei_client_id:{nei_client_id}")
